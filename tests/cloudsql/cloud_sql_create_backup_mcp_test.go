...
func TestCreateBackupToolMCP(t *testing.T) {
	...
	tcs := []struct {
	...
	}{
		{
			...
			args:     map[string]any{"project": "p1", "instance": "instance-no-optional"},
			want:     `{"name":"op1","status":"PENDING"}`,
		},
		{
			name:     "successful backup creation with optional parameters",
			toolName: "create-backup",
			args:     map[string]any{"project": "p1", "instance": "instance-optional", "location": "us-central1", "backup_description": "test desc"},
			want:     `{"name":"op1","status":"PENDING"}`,
		},
		{
